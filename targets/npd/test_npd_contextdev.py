#!/usr/bin/env python3
"""
Test NPD scraper with context.dev Extract API.
...
from targets.npd.scraper import NPDScraper
...
def test_scraper():
    """Test NPD scraper with multiple queries"""

    print_header("NPD Scraper - Context.dev Extract Testing")
    ...
    try:
        scraper = NPDScraper(timeout=60)
        print(f"✅ Initialized NPDScraper with context.dev API")
    ...
    for i, (first, last, city, state) in enumerate(TEST_QUERIES, 1):
        ...
        try:
            ...
            s = state.lower()
            url = f"https://nationalpublicdata.com/people/{letter}/{fn}-{ln}/{s}/{c}/"
            print(f"URL: {url}")
            ...
            if output.summary_results:
                ...
                for j, result in enumerate(output.summary_results):
                    ...
                    if result.addressPreview:
                        print(f"       Address: {result.addressPreview}")
            ...
            if output.profile:
                ...
                print(f"   Age: {output.profile.age}")
                ...
                if output.profile.properties:
                    print(f"   Properties: {len(output.profile.properties)}")
            else:
