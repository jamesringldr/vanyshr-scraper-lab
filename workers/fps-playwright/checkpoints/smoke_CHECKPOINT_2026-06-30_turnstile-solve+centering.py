...
def _frame_rect(fr):
    ...
    None. frame_element()+bounding_box() resolves across cross-origin nesting."""
    try:
...
def _log_frame_tree(page, tag):
    """Ground-truth dump: every frame URL + its iframe rect. So a miss is never
    blind again — the log carries the exact structure."""
    try:
        frames = page.frames
        log(f"  [frames:{tag}] {len(frames)} frame(s):")
        for fr in frames:
            bb = _frame_rect(fr)
            rect = (f"({bb['x']:.0f},{bb['y']:.0f} {bb['width']:.0f}x{bb['height']:.0f})"
                    if bb else "(no-rect)")
            log(f"     {rect}  {(fr.url or '')[:95]}")
    except Exception as e:
...
def handle_turnstile(page, max_wait=70):
    ...
    while time.time() - start < max_wait:
        ...
        # Cheap, non-blocking success signals FIRST. page.url won't block on a
        # heavy/navigating page the way frame/locator calls can (that was the ~2-min
        # hang) — so we detect the pass and EXIT immediately, no more stray clicks.
        try:
            url = page.url
        except Exception:
            url = ""
        # CF issued a clearance token (__cf_chl_f_tk), or we reached a profile (_id_).
        if "__cf_chl_f_tk" in url or "_id_" in url:
            log(f"  [turnstile] cleared — clearance/profile URL (~{elapsed:.0f}s): {url[:70]}")
            return True
        # Success path 2: real results / profile content rendered.
        try:
        ...
        if not cfs:
            # Success path 2: we'd interacted with a challenge and now it's gone.
            if saw_challenge:
                log(f"  [turnstile] challenge frame gone after click (~{elapsed:.0f}s) — cleared")
                return True
            # Never saw a challenge. On the /name/ interstitial or a __cf_chl URL it
            # is probably still loading — keep waiting; otherwise nothing to solve.
        ...
        # Challenge frame(s) present → click the widget checkbox.
        saw_challenge = True
        empty_streak = 0
        target = None
        for fr in cfs:
        ...
        if not target:
            ...
            jitter(1500, 2200)
            continue
        # Checkbox position: inline widget → left ~30px; full-page challenge → the
        # centred widget's checkbox sits left-of-centre.
        if target["width"] > 550:
