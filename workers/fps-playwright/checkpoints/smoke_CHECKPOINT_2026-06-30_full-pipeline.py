...
# Geo-match the fingerprint (timezone/locale/geolocation) to the proxy exit IP.
# Off by default (home-IP runs need no geo lookup). Set GEOIP=1 when proxied so the
# browser's geo agrees with the residential exit IP — Camoufox warns this is needed
# under a proxy. If it 502s on some rotations, fall back to GEOIP=0.
...
def _botcheck_submit(page):
    """FPS's /bot-check landing wraps the Turnstile in its OWN form with a separate
    SUBMIT button — the checkbox alone isn't enough (the inline RESULTS Turnstile
    auto-submits; this landing one does not). Called right after a checkbox click,
    so a challenge IS present (the FPS search form isn't rendered here). Logs every
    button as ground truth, then clicks a submit-ish one. Avoids the site
    logo/search button (we never match "Search"/"FREE SEARCH")."""
    try:
        ...
        for i in range(min(n, 15)):
            try:
                b = btns.nth(i)
                t = (b.inner_text(timeout=200) or b.get_attribute("value") or "").strip()
                labels.append((t or "?")[:28])
            except Exception:
                labels.append("?")
        log(f"  [botcheck] {n} button(s): {labels}")
        for sel in ['button[type="submit"]:visible',
                    'input[type="submit"]:visible',
                    'button:has-text("Submit"):visible',
                    'button:has-text("Verify"):visible',
                    'button:has-text("Continue"):visible',
                    'button:has-text("Proceed"):visible',
                    'form[action*="bot-check"] button:visible']:
            el = page.locator(sel).first
            if el.count() > 0:
                log(f'  [botcheck] clicking submit -> {sel}')
                el.click(timeout=5000)
                return True
    except Exception as e:
        log(f"  [botcheck] err: {e!s:.60}")
    return False
    ...
    try:
        while time.time() - start < max_wait:
            ...
            try:
                ...
                jitter(3500, 5000)                  # let the Turnstile token validate
                # FPS /bot-check landing needs a separate submit click after the
                # checkbox (no-op on the auto-submitting inline results widget).
                _botcheck_submit(page)
                jitter(5000, 8000)                  # let submit + navigation happen
            except Exception as e:
...
def run():
    ...
    cam_kwargs = dict(
        ...
        geoip=GEOIP,             # geo-match fingerprint to exit IP when proxied (GEOIP=1)
    ...
    with Camoufox(**cam_kwargs) as browser:
        ...
        try:
            ...
            # Harvest Cloudflare clearance for the curl-replay experiment: once we've
            # passed the challenge, the context holds cf_clearance/__cf_bm. Dump them
            # + the exact UA so we can test whether the clearance is cookie-portable.
            try:
                cks = context.cookies()
                fps_cks = [c for c in cks if "fastpeoplesearch" in c.get("domain", "")]
                ua = page.evaluate("() => navigator.userAgent")
                has_clr = any(c["name"] == "cf_clearance" for c in fps_cks)
                (OUT_DIR / "cf_cookies.json").write_text(
                    json.dumps({"ua": ua, "url": page.url, "cookies": fps_cks}, indent=2),
                    encoding="utf-8")
                log(f"  harvested {len(fps_cks)} FPS cookies (cf_clearance: {has_clr})")
            except Exception as e:
                log(f"  cookie harvest failed: {e!s:.80}")
